...
export function AdminInterviewersSandbox() {
  ...
  return (
    <div
      className={`${libreFranklin.variable} ${plexSans.variable} min-h-screen bg-slate-50`}
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_20rem] gap-6 items-stretch">
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-center">
                <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-4 px-1">Status totals</p>
                <div className="space-y-3">
...
function AssignmentBucket({
  ...
  return (
    <section className="rounded-3xl border border-[#727D97] bg-[#F7F7F1] p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-3 px-1">
        <p className="text-lg font-bold tracking-tight text-[#111111]" style={{ fontFamily: "var(--font-reports-display)" }}>{title}</p>
        <Badge variant="outline" className="rounded-full border-[#727D97] text-[#5F6C86]">{items.length}</Badge>
      </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <p className="text-sm font-bold text-[#111111]">{item.application_display_id}</p>
                </div>
                {showCurrentOwner && item.current_interviewer ? (
                  <p className="text-[11px] text-[#5F6C86]">
                    Current: <span className="font-semibold text-[#111111]">{item.current_interviewer.name}</span>
                  </p>
                ) : null}
              </div>
              <button className="rounded-full border border-[#727D97] bg-white px-3 py-1.5 text-[11px] font-bold text-[#111111] transition hover:bg-[#E6E9F0]" onClick={() => onAction(item.application_id)} type="button">
                {actionLabel}
              </button>
            </div>
          </div>
...
function getAssignmentClassName(item: AssignmentModalItem) {
  if (item.source === "assigned") return "rounded-[1.4rem] border border-[#198FF0]/25 bg-[#EAF4FD] px-4 py-3";
  if (item.source === "reassign") return "rounded-[1.4rem] border border-[#727D97]/25 bg-white px-4 py-3";
  return "rounded-[1.4rem] border border-[#727D97]/35 bg-white px-4 py-3";
}
