...
def _build_report_chat_messages(question: str, context: dict[str, Any], *, retry: bool = False) -> list[dict[str, str]]:
    system_lines = [
        ...
        "answer_summary must be plain-text prose, not Markdown bullets and not JSON.",
    ]
    if retry:
        system_lines.append(
...
def _recover_partial_report_chat_payload(text: str) -> dict[str, Any]:
...
def answer_report_question(question: str, context: dict[str, Any]) -> ReportChatResponse:
    ...
    attempt_result = _generate_copilot_payload(question, context, retry=False)
    if attempt_result is None or _payload_validation_error(attempt_result, context):
        attempt_result = _generate_copilot_payload(question, context, retry=True)

    if attempt_result is None:
        return _fallback_response(context)

    validation_error = _payload_validation_error(attempt_result, context)
    if validation_error:
        logger.warning("Report copilot validation degraded response: %s", validation_error)
        return _fallback_response(context)
...
def _generate_copilot_payload(question: str, context: dict[str, Any], *, retry: bool) -> Optional[dict[str, Any]]:
    try:
        response_text = generate(
            _build_report_chat_messages(question, context, retry=retry),
    ...
    payload = _recover_partial_report_chat_payload(response_text)
...
def _payload_validation_error(payload: dict[str, Any], context: dict[str, Any]) -> Optional[str]:
    answer = payload.get("answer_summary", "")
    if not answer:
        return "empty answer"
    if _contains_disallowed_judgment(answer):
        return "disallowed judgment"
    return None
