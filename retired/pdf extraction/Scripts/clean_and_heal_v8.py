...
# V8 - DEFINITIVE: Combines all fixes
# - fill=(1,1,1) in add_redact_annot for visual elimination (like V2)
# - clean_contents() for better stream handling  
# - Forced black (0,0,0) healing lines at 0.5pt width
# - Deduplication to prevent double-striking
# - Vertical leak cap to prevent lines leaking into adjacent tables
...
def process_pdf(input_path, output_path, color_threshold=0.1, min_dim=2.0, vert_leak_cap=30.0):
    print(f"--- Processing {input_path} (V8 Definitive) ---")
    
    all_memory = []
    total_removed = 0
    
    # PHASE 1: DEEP STREAM REMOVAL
    ...
    for page in doc:
        ...
        if boxes:
            for rect in boxes:
                # fill=(1,1,1) ensures the area is painted white as part of the 
                # redaction itself. This visually eliminates even boxes that 
                # resist graphics removal, AND provides a clean white background
                # for text injection later.
                page.add_redact_annot(rect, fill=(1, 1, 1))
                all_memory.append((page.number, rect))
            page.apply_redactions(images=0, graphics=1, text=1)
            total_removed += len(boxes)
            print(f"  Page {page.number}: Stripped {len(boxes)} boxes")
    
    print(f"  -> Total boxes processed: {total_removed}")
    
    # Save and reopen for healing
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        ...
        print("Usage: python clean_and_heal_v8.py <input.pdf> <output.pdf>")
